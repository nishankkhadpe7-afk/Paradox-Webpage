...
import {
  ...
  FileText, ShieldAlert, Gift, Camera, Scale, ShieldCheck, HeartHandshake, ArrowUpRight, Lock, Landmark
} from 'lucide-react'
...
const TRACKS = [
  {
    ...
    desc: 'Revolutionize finance with Agentic AI. Think autonomous wealth managers, real-time fraud detection agents, or decentralized lending protocols driven by intelligent agents.',
    features: ['Algo-Trading', 'Risk Analysis', 'Smart Contracts']
  },
  {
    title: 'Cybersec',
    icon: ShieldAlert,
    ...
    desc: 'Forge the next line of defense. Build agents capable of autonomous threat hunting, real-time intrusion response, and predictive vulnerability patching in complex networks.',
    features: ['Threat Hunting', 'Auto-Patching', 'Zero-Trust']
  },
  {
    title: 'GovTech',
    icon: Landmark,
    ...
    desc: 'Optimize the machinery of governance. Create agents for automated grievance handling, smart city resource allocation, or transparent public policy analysis.',
    features: ['Public Policy AI', 'Resource Allocation', 'Transparency']
  }
]
...
const App = () => {
  ...
  const menuVariants = {
    ...
    open: { opacity: 1, y: 15, scale: 1, transition: { staggerChildren: 0.08, delayChildren: 0.1, ease: "easeOut", duration: 0.4 } },
    exit: { opacity: 0, y: -10, scale: 0.98, transition: { staggerChildren: 0.05, staggerDirection: -1 } }
  }
    ...
    <div ref={containerRef} className="relative min-h-screen bg-[#02040a] text-zinc-300 selection:bg-amber-600/30 overflow-x-hidden">
      ...
      <nav className="fixed top-4 md:top-8 left-0 right-0 z-[110] px-4 sm:px-10 flex justify-center pointer-events-none">
        ...
        <div className="lg:hidden flex items-center justify-between w-full max-w-2xl h-14 sm:h-16 px-6 bg-black/70 border border-white/10 rounded-full backdrop-blur-3xl shadow-2xl pointer-events-auto relative">
          ...
          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2.5 text-white active:scale-90 transition-transform relative z-[130] flex items-center justify-center">
                ...
                {navItems.map((item, index) => (
                  ...
                  >
                    <div className="flex items-center gap-4 relative z-10">
                      <span className={`font-phonk text-[9px] opacity-40 ${activeSection === item.id ? 'text-white' : 'text-amber-500'}`}>0{index + 1}</span>
                      <span>{item.label}</span>
                    </div>
                    <ArrowUpRight size={14} className="opacity-20 group-hover:opacity-100" />
                  </motion.button>
                ))}
              </motion.div>
            )}
      ...
      <main className="relative z-10 w-full transform-gpu">
        ...
        <section id="home" className="min-h-[100dvh] flex flex-col items-center justify-center px-6 sm:px-10 text-center pt-32 sm:pt-48 pb-12">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="flex flex-col items-center w-full max-w-5xl">
            <span className="font-phonk text-amber-500/80 tracking-[0.2em] text-[8px] sm:text-xs font-bold uppercase mb-4 md:mb-8">SIESGST ACM CHAPTER PRESENTS</span>
            <div className="relative mb-8 md:mb-12 w-full flex flex-col items-center justify-center">
              <h1 className="text-[clamp(2.5rem,14vw,12rem)] font-phonk text-white uppercase leading-none tracking-tighter drop-shadow-2xl text-center">
                PARA<span className="text-amber-600">DOX</span>
              </h1>
              <p className="mt-4 sm:mt-8 font-phonk text-zinc-400 text-[9px] sm:text-sm font-bold tracking-[0.1em] uppercase leading-relaxed italic opacity-80">CERTAIN ONLY IN UNCERTAINTY</p>
            </div>
            ...
            <motion.button
              ...
              className="px-12 sm:px-24 py-5 sm:py-6 bg-white text-black font-phonk uppercase tracking-[0.2em] rounded-full text-[9px] sm:text-sm hover:bg-amber-500 hover:text-white transition-all duration-300 shadow-2xl mt-4"
            >
              Start Registration
            </motion.button>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-20 items-start">
            <div className="space-y-8 md:space-y-12 font-jakarta text-zinc-300 leading-relaxed font-medium">
              <p className="text-lg sm:text-2xl">
                Paradox is an inter-collegiate Agentic AI Hackathon designed to build autonomous AI-driven solutions. The challenge lies in the "Paradox" — a specific, on-the-spot constraint that will test your ability to adapt and innovate in real-time.
              </p>
              <LiquidGlassCard className="p-8 md:p-10 rounded-[2rem]">
                <p className="text-sm sm:text-lg text-zinc-400 font-semibold italic">The problem statement constraint will be declared on the day to ensure zero pre-built projects.</p>
              </LiquidGlassCard>
            </div>
            <div className="grid grid-cols-2 gap-4 sm:gap-8">
              {[
                ...
                <LiquidGlassCard key={i} className="p-6 sm:p-8 rounded-3xl flex flex-col gap-3">
                  <item.icon size={24} className="text-amber-500" />
                  <div>
                    <p className="font-phonk text-[9px] text-zinc-500 uppercase tracking-widest mb-1">{item.label}</p>
                    <p className="text-xs sm:text-lg font-bold text-white">{item.val}</p>
                  </div>
                </LiquidGlassCard>
              ))}
            </div>
        ...
        <motion.section id="rounds" {...sectionAnimation} className="max-w-7xl mx-auto px-6 sm:px-10 py-16 md:py-32 font-jakarta">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {ROUNDS.map((round, i) => (
              <LiquidGlassCard key={i} className="group relative p-8 md:p-14 rounded-[2.5rem] hover:border-amber-500/30">
                <div className="absolute top-0 right-0 p-8 font-phonk text-5xl md:text-8xl text-white/5 select-none">{round.id}</div>
                <h3 className="font-phonk text-[11px] md:text-xl text-amber-500 uppercase tracking-widest mb-8">{round.title}</h3>
                <div className="space-y-6 relative z-10">
                  <p className="text-sm md:text-xl text-zinc-300 font-medium leading-relaxed">{round.desc}</p>
                  <div className="p-6 rounded-[2rem] bg-white/5 border border-white/5 text-xs md:text-lg text-zinc-400 italic leading-relaxed">{round.eval}</div>
        ...
        {/* --- Judging --- */}
        <motion.section id="judging" {...sectionAnimation} className="max-w-7xl mx-auto px-6 sm:px-10 py-16 md:py-32 font-jakarta">
          <div className="mb-12 md:mb-20 flex flex-col items-center text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10">
            {JUDGING_CRITERIA.map((item, i) => (
              <LiquidGlassCard key={i} className="p-8 md:p-12 rounded-[2rem] hover:border-amber-500/20 transition-all">
        ...
        <motion.section id="tracks" {...sectionAnimation} className="max-w-7xl mx-auto px-6 sm:px-10 py-16 md:py-32 font-jakarta">
          <div className="mb-12 md:mb-20 flex flex-col items-center text-center">
            <h2 className="text-3xl sm:text-6xl md:text-8xl font-phonk text-white uppercase mb-4">Tracks</h2>
            <div className="w-20 sm:w-24 h-1.5 bg-amber-500 rounded-full" />
            <p className="mt-8 text-zinc-400 font-medium max-w-2xl text-center">Select your unit's specialization. Each track demands a unique application of Agentic AI framework.</p>
          </div>
          
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
            {TRACKS.map((track, i) => (
              <LiquidGlassCard key={i} className={`group relative p-8 md:p-12 rounded-[2.5rem] border ${track.border} hover:bg-white/[0.03] transition-all duration-500`}>
                <div className={`absolute inset-0 bg-gradient-to-br ${track.color} opacity-0 group-hover:opacity-100 transition-opacity duration-700`} />
                
                <div className="relative z-10">
                  <div className={`w-14 h-14 md:w-20 md:h-20 rounded-2xl md:rounded-3xl bg-white/5 flex items-center justify-center mb-8 group-hover:scale-110 transition-transform`}>
                    <track.icon size={32} className="text-white group-hover:text-amber-500 transition-colors" />
                  </div>
                  
                  <h3 className="font-phonk text-lg md:text-2xl text-white uppercase tracking-wider mb-6">{track.title}</h3>
                  <p className="text-sm md:text-base text-zinc-400 font-medium leading-relaxed mb-8 min-h-[100px]">{track.desc}</p>
                  
                  <div className="flex flex-wrap gap-2">
                    {track.features.map((feat, idx) => (
                      <span key={idx} className="px-3 py-1.5 rounded-full bg-white/5 border border-white/5 text-[9px] md:text-[11px] font-bold uppercase tracking-widest text-zinc-500 group-hover:text-amber-500 transition-colors">
                        {feat}
                      </span>
                    ))}
                  </div>
        ...
        {/* --- Eligibility & Rules --- */}
        <motion.section id="rules" {...sectionAnimation} className="max-w-7xl mx-auto px-6 sm:px-10 py-16 md:py-32">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <LiquidGlassCard className="p-8 md:p-14 rounded-[2.5rem]">
              <h2 className="text-2xl md:text-4xl font-phonk text-white uppercase mb-10">Protocols</h2>
              <ul className="space-y-6">
                {HACKATHON_RULES.map((rule, i) => (
                  <li key={i} className="flex gap-6 items-start">
                    <div className="w-2 h-2 rounded-full bg-amber-500 mt-2.5 shrink-0" />
                    <p className="text-sm sm:text-lg text-zinc-400 font-medium">{rule}</p>
                  </li>
                ))}
              </ul>
            </LiquidGlassCard>
            <div className="space-y-8">
              <LiquidGlassCard className="p-8 md:p-12 rounded-[2rem]">
                <h3 className="font-phonk text-[10px] md:text-xl text-white uppercase mb-6 tracking-widest">Eligibility</h3>
                <div className="space-y-6">
                  {ELIGIBILITY.map((item, i) => (
                    <div key={i} className="flex gap-4 items-start">
                      <item.icon size={20} className="text-amber-500 shrink-0" />
                      <p className="text-sm text-zinc-400 leading-relaxed"><span className="text-white font-bold">{item.title}:</span> {item.desc}</p>
                    </div>
                  ))}
                </div>
              </LiquidGlassCard>
              <LiquidGlassCard className="p-8 md:p-12 rounded-[2rem]">
                <h3 className="font-phonk text-[10px] md:text-xl text-white uppercase mb-6 tracking-widest">Team Unit</h3>
                <p className="text-sm text-zinc-400 font-medium">3-4 Members per team. Multi-college teams allowed. One member is the primary Lead.</p>
              </LiquidGlassCard>
            </div>
          </div>
        </motion.section>
        ...
        <motion.section id="register" {...sectionAnimation} className="max-w-7xl mx-auto px-6 sm:px-10 py-16 md:py-32">
          ...
          <h2 className="text-[clamp(1.5rem,8vw,7rem)] font-phonk text-white uppercase mb-8 leading-tight tracking-tighter w-full text-center">Registration</h2>
            ...
            <div className="flex flex-col lg:flex-row items-center justify-center gap-8 md:gap-12 relative z-10">
              <motion.a
                href="#" target="_blank" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                className="w-full lg:w-auto px-12 py-6 bg-white text-black font-phonk text-[10px] md:text-lg uppercase tracking-[0.25em] rounded-full hover:bg-amber-500 hover:text-white transition-all shadow-3xl"
              >
                Unstop <ExternalLink size={20} className="inline ml-2" />
              </motion.a>
        ...
        {/* --- Coordinators --- */}
        <motion.section id="coordinators" {...sectionAnimation} className="max-w-7xl mx-auto px-6 sm:px-10 py-16 md:py-32 text-center">
          <div className="mb-12 md:mb-20 flex flex-col items-center text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 max-w-5xl mx-auto">
            {COORDINATORS.map((person, i) => (
              <motion.div key={i} whileHover={{ y: -10 }} className="group">
                <LiquidGlassCard className="p-10 md:p-16 rounded-[3rem]">
                  <div className="relative w-32 h-32 sm:w-56 sm:h-56 mx-auto mb-8 overflow-hidden rounded-full ring-4 ring-white/5 group-hover:ring-amber-500/30 transition-all duration-500">
                  ...
                  <p className="text-[10px] md:text-sm text-amber-500 font-bold uppercase tracking-[0.4em] mb-8">{person.role}</p>
                  <a href={person.linkedin} className="inline-block p-4 rounded-full bg-white/5 hover:bg-amber-600 transition-all">
      ...
      {/* --- Footer --- */}
      <footer className="py-20 text-center border-t border-white/5 bg-black/90 backdrop-blur-xl font-jakarta relative z-20">
        ...
        <div className="flex justify-center gap-8 mb-12 opacity-60">
          {[Instagram, Twitter, Linkedin].map((Icon, i) => (
            <a key={i} href="#" className="text-white hover:text-amber-500 hover:scale-125 transition-all">
              <Icon className="w-6 md:w-8 h-6 md:h-8" />
            </a>
          ))}
        </div>
        <div className="px-6 space-y-4">
          <p className="text-zinc-500 text-[10px] md:text-[13px] uppercase font-bold tracking-[0.4em] opacity-60 leading-loose">SIESGST ACM CHAPTER // EST. 2026 // NAVI MUMBAI </p>
      ...
      <style>{`
        ...
        html { scroll-behavior: smooth; overflow-y: scroll; scroll-padding-top: 100px; }
        body { background-color: #02040a; color: white; overflow-x: hidden; margin: 0; font-family: var(--font-jakarta); -webkit-font-smoothing: antialiased; }
